...
public class EnemyBehavior2 : MonoBehaviour
{
    ...
    void State_EveryFrameBehavior()
    {
        ...
        switch (enemyState)
        {
            ...
            case EnemyState.FarCombat:
                ...
                MoveAlongPath();
                GetComponent<EnemyGunHandler>().TryToShoot();
    ...
    GameObject CheckForPlayersInCone()
    {
        ...
        List<Collider2D> playersInConeAndLineOfSight = new List<Collider2D>();
        ...
        foreach (Collider2D playerCollider in playersInCone)
        {
            if (playerCollider)
            {
                ...
                if (!hitInfo)
                {
                    playersInConeAndLineOfSight.Add(playerCollider);
                }
            }
        }
        ...
        foreach (Collider2D playerCollider in playersInConeAndLineOfSight)
        {
    ...
    {
        if(!isLookingAtDesiredRotation)
            rb.rotation = (Quaternion.RotateTowards(transform.rotation, Quaternion.Euler(0, 0, desiredRotation), rotSpeed * Time.deltaTime)).eulerAngles.z;

        if (Mathf.Abs(Mathf.DeltaAngle(rb.rotation,desiredRotation))<.1f)
